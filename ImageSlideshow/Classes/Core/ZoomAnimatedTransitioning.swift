...
open class ZoomAnimatedTransitioningDelegate: NSObject, UIViewControllerTransitioningDelegate {
    ...
    func initialize() {
        ...
        UIApplication.shared.keyWindowInConnectedScenes?.addGestureRecognizer(gestureRecognizer)
    }
...
class ZoomOutAnimator: ZoomAnimator, UIViewControllerAnimatedTransitioning {
    ...
    private func animationParams(using transitionContext: UIViewControllerContextTransitioning) -> (TimeInterval, () -> Void, (Any) -> Void) {
        ...
        if let referenceImageView = referenceImageView {
            ...
            if isStatusBarHidden, !toViewController.prefersStatusBarHidden, referenceSlideshowViewFrame.origin.y != parent.referenceSlideshowViewFrame?.origin.y {
        ...
        let completion = { (_: Any) in
            ...
            if completed {
                ...
                UIApplication.shared.keyWindowInConnectedScenes?.removeGestureRecognizer(self.parent.gestureRecognizer)
                // Unpauses slideshow
